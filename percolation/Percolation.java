...
public class Percolation {
    ...
    public void open(int row, int col) {
        ...
        if (row == 1) { quickFind.union(indexPosition(row, col), t); }

        if (row == size) { quickFind.union(indexPosition(row, col), b); }
        if (row > 1 && isOpen(row - 1, col)) { quickFind.union(indexPosition(row, col), indexPosition(row - 1, col)); }
    ...
    private int indexPosition(int row, int col) { return size * (row - 1) + col; }
